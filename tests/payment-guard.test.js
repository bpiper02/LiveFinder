...
assert.equal(guard.actionDecision('No thanks', 'Featured placement $1000').decision, 'safe-free');
...
assert.equal(guard.actionDecision('Next', '').decision, 'neutral');
...
assert.notEqual(guard.classifySurfaceText('Creator feature list $10').kind, 'optional-upsell', 'generic feature copy must not create a fake add-on surface');
